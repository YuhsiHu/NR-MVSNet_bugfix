...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    loss = model_loss(outputs, depth_gt_ms, mask_ms, update_loss=args.update_loss, dlossw=[float(e) for e in args.dlossw.split(",") if e])
...
def test_sample(model, model_loss, sample, args):
    ...
    outputs = model_eval(sample_cuda["imgs"], sample_cuda["proj_mats"], sample_cuda["depth_values"])
    depth_est = outputs["depth"]
...
if __name__ == '__main__':
    ...
    test_dataset = MVSDataset(args.testpath, "test", 5)
    train_dataset = MVSDataset(args.trainpath, "train", 3)
