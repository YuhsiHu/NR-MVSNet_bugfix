...
class CostRegNet(nn.Module):
    ...
    def forward(self, x):
        conv0 = self.conv0(x) # C
        conv2 = self.conv2(self.conv1(conv0)) # 2C
        conv4 = self.conv4(self.conv3(conv2)) # 4C
        x = self.conv6(self.conv5(conv4)) # 8C
        x = conv4 + self.conv7(x) # 4C
        x = conv2 + self.conv9(x) # 2C
        x = conv0 + self.conv11(x) # C
        x = self.prob(x) # 1
        return x
...
def mvsnet_loss(inputs, depth_gt_ms, mask_ms, **kwargs):
    depth_loss_weights = kwargs.get("dlossw", None)
    update_loss_weight = kwargs.get("update_loss", 0)
    ...
    for (stage_inputs, stage_key) in [(inputs[k], k) for k in inputs.keys() if "stage" in k]:
        ...
        depth_loss_weight_stage = depth_loss_weights[stage_idx] if depth_loss_weights is not None else 1.0

        if stage_idx == 0:
            update_est = stage_inputs['depths'][1]
            update_depth_loss = F.smooth_l1_loss(update_est[mask], depth_gt[mask], reduction='mean')
            total_loss += update_loss_weight * update_depth_loss

            iters = len(stage_inputs["depths"])
            for iter in range(iters):  
                if iter == 1: continue
                depth_est = stage_inputs["depths"][iter]
                depth_loss = F.smooth_l1_loss(depth_est[mask], depth_gt[mask], reduction='mean')
                total_loss += depth_loss_weight_stage * depth_loss
        else:
            iters = len(stage_inputs["depths"])
            for iter in range(iters):  
                depth_est = stage_inputs["depths"][iter]
